...
public class Results extends AppCompatActivity implements View.OnClickListener {
    ...
    private void detectAdtrt() {
        //dummy data to detect adulterant based on received pH value from arduino
        if (pH > 6.7) {
    ...
    private void saveRecord(String sourceAddress, String adulterant) {
        if (submitButton) {
            ...
            Toast.makeText(getApplicationContext(), "Failed to save adulterant record", Toast.LENGTH_SHORT).show();
        }
    }
}